...
void myLine::drawAt(int topY, int drawRadius, bool erase) {
    // Build the list of symbol positions for this frame.
    symbols.clear();
    for (int row = 0; row < drawRadius; row++) {
        ...
        if (single) {
            if (centerX >= 0 && centerX < csWidth) {
                symbols.push_back(SymbolPos{ centerX, y });
            }
        }
        else {
            int xLeft = centerX - 1;
            ...
            if (xLeft >= 0 && xLeft < csWidth) {
                symbols.push_back(SymbolPos{ xLeft, y });
            }
            if (xRight >= 0 && xRight < csWidth) {
                symbols.push_back(SymbolPos{ xRight, y });
            }
        }
    }
    ...
    // Render using the linked list of positions.
    for (size_t i = 0; i < symbols.size(); ++i) {
        const SymbolPos& pos = symbols.at(i);
        if (erase) symb.deleteSymbol(pos.x, pos.y);
        else symb.printSymbol(pos.x, pos.y);
    }
}

void myLine::update(ULONGLONG nowMs, int explosionChance, int minExplosionRadius, int maxExplosionRadius, LinkedList<Explosion>& explosions) {
    if (finished) return;
    if (!started) {
        started = true;
        moveTimes.clear();
        moveIndex = 0;
        // Start ms schedule for line animation.
        moveTimes.push_back(nowMs);
    }

    while (!finished && moveIndex < moveTimes.size() && nowMs >= moveTimes[moveIndex]) {
        ...
        if (doExplode) {
            ...
            explosions.push_back(Explosion(csWidth, csHeight, centerX, bottomY, minExplosionRadius, maxExplosionRadius));
        ...
        if (!finished) {
            moveTimes.push_back(moveTimes[moveIndex - 1] + velocityMs);
        }
    }
}
